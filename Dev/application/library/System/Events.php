...
class Events
{
    ...
    public function trigger($name, $params=array())
    {
        ...
        if(isset($this->_binds[$name])) {
            foreach($this->_binds[$name] as $weight => $hooks) {
                foreach($hooks as $hookName => $callback) {
                    $responce[$hookName] = call_user_func_array($callback, $params);
                }
            }
        }
        return $responce;
     ...
     */
    public function bind($eventName, $hookName, $callback, $weight=50)
    {
        ...
        if(isset($this->_binds[$eventName])) {
            foreach($this->_binds[$eventName] as $wht => $hooks) {
                if(isset($this->_binds[$eventName][$wht][$hookName])) {
                    unset($this->_binds[$eventName][$wht][$hookName]);
                }  
            }
        }
        $this->_binds[$eventName][$weight][$hookName] = $callback;
        ksort($this->_binds[$eventName]);
        return true;
    }
    ...
    public function unbind($eventName, $hookName = null)
    {
        if(null === $hookName) {
            ...
            foreach($this->_binds[$eventName] as $weight => $hooks) {
                if(isset($this->_binds[$eventName][$weight][$hookName])) {
                    unset($this->_binds[$eventName][$weight][$hookName]);
                    return true;
                }            
            }
        }
     ...
     * @throws \InvalidArgumentException
     */
    public function addFilter($eventName, $hookName, $callback, $weight=50)
    {
        if(!is_callable($callback)) {
            throw new \InvalidArgumentException("3rd parameter must be valid callback. Given (" . gettype($callback) . ")");
        }
        $this->_filters[$eventName][$weight][$hookName] = $callback;
        ksort($this->_filters[$eventName]);
        return $this;
    }
    ...
    {
        if(isset($this->_filters[$name])) {
            foreach($this->_filters[$name] as $hookName => $callbacks) {
