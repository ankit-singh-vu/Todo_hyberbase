...
class Template extends Module\Response
{
    ...
    {
        $this->_scriptParams = array_merge_recursive($this->_scriptParams, $params);
    }
     ...
     * @return System\View\Template\Block
     */
    ...
    {
        $helperClass = 'System\View\Generic\\' . ucfirst($name);
     ...
     * @return System\View\Helper\HelperAbstract
     */
    public function helper($name)
    {
        $kernel = \Kernel();
        //_debug($kernel->loader()->getNamespacePaths('Module'));
        $helperClass = false;
        foreach($kernel->loader()->getNamespacePaths('Module') as $modulePath) {
            if(file_exists($modulePath . '/Module')) {
                foreach (scandir($modulePath . '/Module') as $module) {
                    if (!in_array($module, array('.', '..'))) {
                        if(file_exists($modulePath . '/Module/' . $module . '/Helper/' . ucfirst($name) . '.php')) {
                            $helperClass = 'Module\\'. $module .'\Helper\\' . ucfirst($name);
                            break;
                        }
                    }
                }
        ...
        if($helperClass == false) {
            if (file_exists($kernel->config('app.path.lib') . '/App/View/Helper/' . ucfirst($name) . '.php')) {
                $helperClass = '\App\View\Helper\\' . ucfirst($name);
            } else {
                $helperClass = '\System\View\Helper\\' . ucfirst($name);
            }
        }
    ...
    /**
     * Load and return new system view for partial
     *
     ...
     * @throws System\View\Exception\TemplateMissing
     * @return boolean
     */
    ...
    /**
     * Stops further rendering od template
     ...
     * Supports Array and \ActiveRecord\Model
     *
     * @param $data
     * @return bool
     */
    public function sendJson($data)
    {
        ...
        if($data instanceof \ActiveRecord\Model) {
            echo $data->to_json();
            $this->stopRender();
        }
        if(is_array($data) && isset($data[0]) && $data[0] instanceof \ActiveRecord\Model) {
            $responce = array();
            foreach($data as $key => $model) {
                $responce[$key] = $model->to_array();
            }
    ...
    public function content($parsePHP = true)
    {
        ...
        if($parsePHP) {
            ...
            extract($this->vars());
            /*if($kernel->request()->isAjax()) {
                echo $this->head()->content() . "\n";
            }*/
            include($vfile);
