...
class Validations
{
     ...
     * @return array|string
     */
    protected function verify_email_existence($toemail, $fromemail='dc@mclogics.com', $getdetails = true){
    ...
    /**
     * Validates a field value for email address
     *
     * @param $attrs
     */
	public function validates_is_email($attrs)
	{
        foreach ($attrs as $attr)
        {
            $value = $this->model->$attr[0];
            if(!isset($attr['message'])) {
                $attr['message'] = '"' . $value . '" is not a valid email address';
            }
            if(!isset($attr['non_existence'])) {
                $attr['non_existence'] = '"' . $value . '" is either invalid or not reachable';
            }
            if (!filter_var($value, FILTER_VALIDATE_EMAIL)) {
                $this->record->add($attr[0], $attr['message']);
            } else {
                if(isset($attr['check_remote_server'])) {
                    $verify = $this->verify_email_existence($value);
                    if($verify[0] == 'invalid') {
                        $this->record->add($attr[0], $attr['non_existence']);
                        if(isset($attr['show_remote_error']) && $attr['show_remote_error'] == true) {
                            $this->record->add($attr[0], $verify[1]);
                        }
                    }
                }
            }
